...
namespace ImageProcessing
{
    public static class ImageDecoder
    {
        ...
        public static void decode_image(Bitmap image, int user_id, uint image_id, String url)
        {
            ...
            if (key != "2")
            {
                ...
                saveImage(image, user_id, image_id, url);
            }
        ...
        private static void saveImage(Bitmap image,int user_id, uint image_id, String url)
        {
            String time = DateTime.Now.ToString("dd-MM-yyyy");
            if (!Directory.Exists("DecodedImages\\"+time))
                Directory.CreateDirectory("DecodedImages\\"+time);

            string loc = "DecodedImages\\" + time + "\\" + Convert.ToString(user_id) + Convert.ToString(image_id) + time+".jpg";
            image.Save(loc);
